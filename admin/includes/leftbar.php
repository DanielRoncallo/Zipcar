	<nav class="ts-sidebar">
			<ul class="ts-sidebar-menu">
			
				<li class="ts-label">Main</li>
				<li><a href="dashboard.php"><i class="fa fa-dashboard"></i> Dashboard</a></li>
			
<li><a href="#"><i class="fa fa-files-o"></i> Marcas</a>
<ul>
<li><a href="create-brand.php">Crear Marca</a></li>
<li><a href="manage-brands.php">Manejar Marcas</a></li>
</ul>
</li>

<li><a href="#"><i class="fa fa-sitemap"></i> Vehiculos</a>
					<ul>
						<li><a href="post-avehical.php">Publicar un Vehiculo</a></li>
						<li><a href="manage-vehicles.php">Manejo Vehiculos</a></li>
					</ul>
				</li>
				<li><a href="manage-bookings.php"><i class="fa fa-users"></i> Manejo Reservas</a></li>

				<li><a href="testimonials.php"><i class="fa fa-table"></i> Manejo Testimonios</a></li>
				<li><a href="manage-conactusquery.php"><i class="fa fa-desktop""></i> Manage Conatctus Query</a></li>
				<li><a href="reg-users.php"><i class="fa fa-users"></i> Reg Users</a></li>
			<li><a href="manage-pages.php"><i class="fa fa-files-o"></i> Manage Pages</a></li>
			<li><a href="update-contactinfo.php"><i class="fa fa-files-o"></i> Update Contact Info</a></li>

			<li><a href="manage-subscribers.php"><i class="fa fa-table"></i> Manage Subscribers</a></li>

			</ul>
		</nav>